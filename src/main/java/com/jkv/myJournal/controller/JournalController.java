...
import com.jkv.myJournal.entity.JournalEntries;
...
public class JournalController {
     ...
     */
    private Map<Long, JournalEntries> journalEntriesMap = new HashMap<>();
     ...
     */
    @PostMapping
    public Boolean postEntry(@RequestBody JournalEntries entry) {
     ...
     */
    @GetMapping("/search")
    public List<JournalEntries> getEntryByParam(@RequestParam Long id) {
        List<JournalEntries> list = new ArrayList<>();
        JournalEntries entry = journalEntriesMap.get(id);
        if (entry != null) {
            list.add(entry);
        }
        return list;
     ...
     */
    @PutMapping("/id/{myId}")
    public boolean putEntry(@RequestBody JournalEntries entry, @PathVariable long myId) {
